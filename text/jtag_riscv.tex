...
Tato kapitola popisuje princip přenosu dat pomocí komunikačního protokolu JTAG a jeho použití pro testování obvodů s procesorem architektury \acs{RISC-V} (\textit{\acl{RISC-V}}). V kapitole je také přiblížen princip fungování systémové sběrnice použitého \acs{RISC-V} procesoru.
...
Na sběrnici může nastat také případ, kdy paměť nemůže odpovědět okamžitě a signál \texttt{data\_gnt\_i} je nastaven až po několika hodinových taktech, jak je vidět na obrázku \ref{fig:ri5cy_bus_slow_res}. V tomto případě musí být pouze zaručeno, aby \textit{master} neměnil signály \texttt{data\_we\_o}, \texttt{data\_be\_o}, a \texttt{data\_wdata\_o}. 
