...
\usepackage{booktabs}		% hodí se pro změnu tloušťky tabulek
\usepackage{arydshln}		% hodí se pro změnu tloušťky tabulek
\usepackage{xcolor}		% hodí se pro změnu tloušťky tabulek
...
%% Vložení zadání
\includepdf[pages=1]%   buďto generovaného informačním systémem
  {pdf/zadani_dip}% název souboru nesmí obsahovat mezery!
...
\include{text/jtag_riscv}

%Vlozeni souboru s druhou kapitolou (JTAG rozhraní 2w)
\include{text/jtag_2w}

%Vlozeni souboru s třetí kapitolou (Advanced protocol)
\include{text/jtag_ap}

%Vlozeni souboru se čtvrtou kapitolou (Software)
\include{text/sw}
